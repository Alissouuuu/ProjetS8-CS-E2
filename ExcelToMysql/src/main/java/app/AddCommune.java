...
public class AddCommune {
    ...
    public static void main(String[] args) {
        ...
        String nomFichierCommune = pr.getProperty("nomFichierCommune"); // Fichier CSV contenant les communes
        ...
        try {
            ...
            BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(nomFichierCommune), "UTF-8"));
            // Nombre de lignes insérées dans la BDD
            ligne = br.readLine();
            String requeteCommune = "INSERT INTO " + nomTableCommune + " (code_commune, code_departement,latitude,longitude,location) VALUES (?, ?, ?, ?, POINT(?,?))";
            stmt = con.prepareStatement(requeteCommune);  // Préparer la requête d'insertion dans la table `commune`
            boolean mauvaiseLigne;
            String code_commune = null,code_departement=null,latitudeString=null,longitudeString=null;
                    ...
                    if (ligne != null) {
                        ...
                        mauvaiseLigne = false;
                        	try {
                                ...
                                code_departement = tab[11].trim(); // Extraire le code_departement (12e colonne)
                                latitudeString = tab[5].trim(); // Extraire la latitude de la commune (6e colonne)
                                longitudeString = tab[6].trim(); // Extraire la longitude de la commune (7e colonne)
                                
                                code_commune = enleverGuillemets(code_commune);
                                code_departement = enleverGuillemets(code_departement);
                                latitudeString = enleverGuillemets(latitudeString);
                                longitudeString = enleverGuillemets(longitudeString);
                            ...
                            if(mauvaiseLigne || latitudeString == null || latitudeString.isEmpty() || longitudeString == null || longitudeString.isEmpty()) {
                            	System.out.println("latitude ou longitude vide !");
                            }
                            else {
	                            ...
	                            // Vérifier si le code_departement existe dans la table departement
	                            if (codeDepartementExiste(con, code_departement)) {
	                                ...
	                                if (!communeExiste(con, code_commune)) {
	                                    ...
	                                    stmt.setString(1, code_commune);  // on inscrit le code_commune dans la requête
	                                    stmt.setString(2, code_departement);  // pareil pour le code département
	                                    stmt.setDouble(3, latitude);  // pareil pour la latidude
	                                    stmt.setDouble(4, longitude);// pareil pour la longitude
	                                    stmt.setDouble(5, latitude); //pareil pour la latidude, cette fois-ci pour la localisation
	                                    stmt.setDouble(6, longitude);//pareil pour la longitude, cette fois-ci pour la localisation
	                                    ...
	                                    System.out.println("Commune déjà existante dans la base de données : " + code_commune);
	                                }
	                            } else {
	                                System.out.println("Le code de département " + code_departement + " n'existe pas dans la table `departement`.");
	                            }
                    		}
                    }
    ...
    // Vérifie si le code_departement existe dans la table 'departement'
    public static boolean codeDepartementExiste(Connection conn, String code_departement) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                return rs.next(); // Retourne true si le code_departement existe dans la table departement
            }
        }
    }
