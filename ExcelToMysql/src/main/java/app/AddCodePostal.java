...
public class AddCodePostal {
    ...
    final static String nomTableCodePostal = "code_postal"; // Nom de la table des communes
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            ligne = br.readLine();
            String requeteCommune = "INSERT INTO " + nomTableCodePostal + " (code_postal,code_commune) VALUES(?,?)";
            stmt = con.prepareStatement(requeteCommune);  // Préparer la requête d'insertion dans la table `commune`
            String code_commune = null,code_postal=null;
            ...
            do {
                try {
                    ...
                    if (ligne != null) {
                        ...
                        code_postal = tab[2].trim();
                        
                        code_commune = enleverGuillemets(code_commune);
                        code_postal = enleverGuillemets(code_postal);
                        ...
                        // Vérifier si le code_commune existe dans la table commune
                            if (communeExiste(con,code_commune) && !codePostalExiste(con,code_commune,code_postal)) {
                                // Insérer la commune dans la base de données
                                stmt.setString(1, code_postal);  // on inscrit le code postal dans la requête
    ...
    public static boolean codePostalExiste(Connection conn, String code_commune, String code_postal) throws SQLException {
        String sql = "SELECT 1 FROM " + nomTableCodePostal + " WHERE code_commune = ? AND code_postal = ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, code_commune);
            stmt.setString(2, code_postal);
            try (ResultSet rs = stmt.executeQuery()) {
                return rs.next(); // Retourne true si la combinaison existe déjà
            }
        }
    }
